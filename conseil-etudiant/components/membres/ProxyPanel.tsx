...
import { createProxy, deleteProxy, getTeamMembers, inlineProxyUrl, setProxyValidation } from "@/lib/members-api"
...
export default function ProxyPanel({
    ...
    // Un membre ne déclare que la procuration qu'il porte lui-même, une seule fois.
    // Le président de séance et le bureau encodent en plus pour les autres : ils ne
    // deviennent pas porteurs, ils saisissent à la place du mandataire désigné.
    ...
    // Le mandant ne peut pas être le mandataire, ni quelqu'un de déjà engagé.
    const grantorCandidates = members.filter(
        (m: any) => m.email
    ...
    // Changer de mandataire peut rendre le mandant sélectionné invalide.
    useEffect(() => {
                ...
                <div className="border border-primary/30 bg-primary/5 rounded-2xl p-5 space-y-4">
                    <div>
                        ...
                        <p className="text-xs text-muted-foreground mt-1">
                            Désignez le membre absent qui confie sa voix et joignez sa procuration signée (PDF).
                    ...
                    <div className="space-y-1.5">
                        ...
                        >
                            ...
                            {grantorCandidates.map((m: any) => (
                                <option key={m.email} value={m.email}>
                                    {m.name}{m.campus?.name ? ` — ${m.campus.name}` : ""}
                                </option>
                            ))}
                        </select>
                        {members.length > 0 && grantorCandidates.length === 0 && (
                            <p className="text-xs text-muted-foreground">
                        ...
                        return (
                            ...
                            {previewing && p.fileUrl && (
                                <div className="px-4 pb-4 space-y-2 border-t border-border pt-3">
                                    ...
                                    <PdfPreview url={inlineProxyUrl(p.id)} />
