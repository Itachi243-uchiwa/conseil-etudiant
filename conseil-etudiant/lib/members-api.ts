...
/**
 * Ordre du jour d'une séance. Les horaires et la numérotation sont calculés par
 * le serveur à partir de l'heure de la séance et des durées : le client se
 * contente d'afficher ce qu'il reçoit.
 */
...
/** Le président coche un point traité : les membres voient où en est la séance. */
...
export async function deleteAgendaItem(itemId: number, email: string, name: string) {
    return memberFetch(`/members/agenda/${itemId}`, email, name, { method: "DELETE" })
}
 ...
 * navigateur à télécharger le fichier au lieu de l'afficher. Le backend expose
 * un relais qui le renvoie en `application/pdf` inline : c'est cette URL qu'il
 * faut donner à un <object> ou un <iframe>.
 */
export function inlineDocumentUrl(documentId: number) {
    return `${BASE}/members/documents/${documentId}/inline`
}

export function inlineProxyUrl(proxyId: number) {
    return `${BASE}/members/proxies/${proxyId}/inline`
}
...
export async function createProxy(
    ...
    name: string,
    /**
     * Mandataire désigné sur le PDF, quand il n'est pas la personne connectée.
     * Réservé au président de séance et au bureau : c'est le backend qui le
     * vérifie. Le porteur reste celui-ci, jamais l'encodeur.
     */
    holderEmail?: string
) {
